...
class ApiTestCase(unittest.TestCase):
    ...
    def test_6_train_score(self):
        ...
        resp = self._request('GET', '/trunc_table/')
    ...
    def test_7_groups(self):
        """ test model groups """
        d = 4
        models = [
            {'model': 'MLPClassifier', 'n_trained': 4},
            {'model': 'CategoricalNB', 'n_trained': 2},
            {'model': 'MLPClassifier', 'n_trained': 13},
            {'model': 'SGDClassifier', 'n_trained': 13},
        ]

        model_id_groups = defaultdict(list)
        ...
        for model in models:
            ...
            model_id = resp['id']
            model_id_groups[model['n_trained']].append(model_id)
            # train
            X = numpy.abs(numpy.random.randn(model['n_trained'], d))
            W = numpy.random.randn(d)
            Y = 1 + X.dot(W) > 0
        ...
        resp_model_id_groups = {group['n_trained']: group['model_ids'] for group in resp['groups']}
        for n_trained, resp_models_ids in model_id_groups.items():
